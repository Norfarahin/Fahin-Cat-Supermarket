...
if(isset($_GET['delete'])){
    ...
    mysqli_query($conn, "DELETE  FROM `cart1` WHERE id = '$delete_id'") or die('query failed');
...
<section class="shopping-cart">
    ...
    <?php
        $grand_total = 0;
        $select_cart = mysqli_query($conn, "SELECT cart1.id as cartid, product.id as pid, product.name, product.price, product.image, cart1.quantity  FROM `cart1`  JOIN `customer` ON cart1.user_id = customer.id JOIN product on cart1.pid=product.id WHERE user_id = '$custid'") or die('query failed');
